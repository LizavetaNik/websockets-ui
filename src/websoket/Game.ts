import { AppWebSocket, IRoom, IPlayer, IShip, ICell } from '../data/interface';
...
export class Game {
  ...
  private resultAttack(competitorPlayer: IPlayer, x: number, y: number) {
    const cell = competitorPlayer.game?.[x]?.[y];
    if (!cell || cell.isAttacked) {
      return;
    }

    cell.isAttacked = true;
    if (cell.shipIndex === -1) {
      return { status: 'miss', shipIndex: cell.shipIndex };
    }
  ...
  createGame(roomId: number) {
    roomsAll.get(roomId)?.players?.forEach(
      ({ ws, id }) =>
        ws &&
        this.sendMessage(ws, 'create_game', {
          idGame: roomId,
          idPlayer: id,
        }),
    );
  ...
  createRoomForGame(ws: AppWebSocket) {
    ...
    return { roomId: newRoomId, Room: roomsAll.get(newRoomId) as IRoom };
  }
  ...
  addShips(dataString: string) {
    ...
    ships.forEach((ship: IShip, id: number) => {
      ship.hp = ship.length;
    ...
    if (players.every((player) => !!player.ships)) {
      players.forEach(
        ({ ws, ships }) =>
          ws &&
          this.sendMessage(ws, 'start_game', {
  ...
  createSinglePlay(ws: AppWebSocket) {
    ...
    if (roomThisGame && roomThisGame.players.length === 2) {
      return;
    }
    ...
    if (!currentRoom) {
      const roomId = this.createRoomForGame(ws)?.roomId as number;
      currentRoom = roomsAll.get(roomId) as IRoom;
    }
    currentRoom.players = [currentRoom.players[0] as IPlayer, bot];
  ...
  attack(dataString: string) {
    ...
    const roomThisGame = roomsAll.get(gameId);
    const players = roomThisGame?.players;
    if (!roomThisGame || roomThisGame.idPlayerCurrent !== indexPlayer) {
      return;
    }
    ...
    this.nextStep(
      gameId,
      attackResult.status === 'miss' ? competitorId : undefined,
    );
  }
  ...
  gameOver(players: IPlayer[], winnerPlayerIndex: number) {
    players.forEach(({ ws }) => {
      ws && this.sendMessage(ws, 'finish', { winPlayer: winnerPlayerIndex });
    });
  }

  createBotPlayer(): IPlayer {
    const ships: IShip[] = [];
    ...
    const board: ICell[][] = Array.from({ length: gridSize }, () =>
